...
# Source: laya/email.py (https://github.com/NandhaKishorM/laya, licence text in LICENSES/Apache-2.0.txt).
# Changes: clean_email_body renamed clean_email; the email_questions re-export from laya.presets dropped; module and
# function docstrings rewritten for Tez; type hints modernised. The cleaning rules and their order are Laya's.
"""Helpers that turn raw inputs into good states. An email is decided on the new message only: quoted history, the
signature and confidentiality footers are removed, because the quoted history (often a different request) otherwise
weighs on the answer as much as the new message does.

    from tez.state import email_state
    tez.decide(email_state("Re: invoice 4411", raw_body, sender="ana@example.com"), schema="support-triage")

The markers cover English, Portuguese and Spanish mail clients (Gmail, Outlook and mobile footers). Cleaning errs
towards keeping text: dropping the sender's request is silent and severe, leaving one boilerplate line behind is not.
"""
...
def _starts_new_sentence(line: str) -> bool:
    """First letter is uppercase: a fresh sentence, not a wrapped line. Lines in uncased scripts (CJK, Devanagari,
    ...) never start a new piece, so wrapped boilerplate in those scripts still drops whole."""
    for ch in line:
        if ch.isalpha():
            return ch.isupper()
    return False
...
def _split_fused_lines(sentence: str) -> list[str]:
    """Split a fused boilerplate-positive sentence at sentence-starting newlines: an unpunctuated request line glued
    to a disclaimer line ("locked\\nThis ...") splits because the next line starts uppercase; a lowercase
    continuation ("are\\nconfidential") belongs to the same sentence, so a wrapped footer still drops whole."""
    if "\n" not in sentence:
        return [sentence]
    pieces, buf = [], ""
...
def _strip_disclaimer(paragraph: str) -> str:
    """Drop boilerplate disclaimer sentences from one paragraph. A paragraph goes whole only when every sentence in it
    is boilerplate; a footer that runs on without a blank line must not take the sender's request with it."""
...
def clean_email(body: str | None, max_chars: int = 3000) -> str:
    """The new message of an email body: quoted history, signatures, device footers and confidentiality disclaimers
    removed (English, Portuguese and Spanish mail clients), whitespace tidied, at most `max_chars` characters."""
    text = (body or "").replace("\r\n", "\n").replace("\r", "\n").replace("\\n", "\n")
    # Bound regex work before the expensive patterns below: _DISCLAIMER's bounded repeats cost more on long input,
    # and only max_chars are ever returned.
...
def email_state(subject: str | None, body: str | None, sender: str | None = None, clean: bool = True,
                **extra: Any) -> dict:
    """A state for deciding on an email: {"subject", "body", "from"?, **extra}, the body cleaned with clean_email
    (clean=False keeps it as it is). Extra fields that are None are left out."""
    state: dict[str, Any] = {
        "subject": (subject or "").strip(),
        "body": clean_email(body) if clean else (body or ""),
    }
    if sender:
        state["from"] = sender
...
__all__ = ["clean_email", "email_state"]
