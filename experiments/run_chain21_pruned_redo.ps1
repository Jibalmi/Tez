# Chain 21 (GPU, llama.cpp): rerun of the pruned-GGUF benchmark and the 29-block head-to-head with prompt caching OFF and the
# end-of-sequence retry in bench_h2h.tez_score_letters (depth-pruned models sometimes put EOS first). Queued behind chain 20.
...
$log = "$env:TEMP\abl21"; New-Item -ItemType Directory -Force $log | Out-Null
while (-not (Test-Path "$env:TEMP\abl20\done.txt")) { Start-Sleep 20 }
Start-Sleep 20
Get-Process llama-server -ErrorAction SilentlyContinue | Stop-Process -Force; Start-Sleep 5
if (Test-Path "results\h2h_q4b_L29") { Remove-Item -Recurse -Force "results\h2h_q4b_L29" }
...
$p = Start-Process -FilePath $exe -WorkingDirectory "C:\temp\llamacpp" -ArgumentList "-m $root\tools\models\Qwen3.5-4B-Q8_0-L29.gguf -ngl 99 -c 8192 -b 512 --port 8092 --host 127.0.0.1 -np 1 --no-webui" -WindowStyle Hidden -PassThru -RedirectStandardError "$log\server_L29.err" -RedirectStandardOutput "$log\server_L29.out"
Start-Sleep 25
cmd /c "python experiments\bench_h2h.py --model tez --tasks all --server http://127.0.0.1:8092 --out results\h2h_q4b_L29 > $log\h2h_L29.txt 2>&1"
...
"CHAIN21DONE $(Get-Date)" | Out-File "$log\done.txt"
